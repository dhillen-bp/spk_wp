    ...
    <div class="w-full rounded-lg border border-base-content/25 px-4 py-2 text-sm">
        <div class="breadcrumbs">
            <ul>
                ...
                <li class="breadcrumbs-separator rtl:-rotate-[40deg]">/</li>
                <li aria-current="page">Update Employe Score</li>
            </ul>
        </div>
    </div>
    <h3 class="text-lg">Update Employe Score</h3>
    <form wire:submit="save">
        <div class="form-control mb-6 md:w-1/2">
           ...
           @enderror select" id="employee"
                ...
                @foreach ($employees as $employee)
                    <option {{ $form->employee_id == $employee->id ?? 'selected' }} value="{{ $employee->id }}">
        ...
        @foreach ($criterias as $criteria)
            <div class="form-control mb-6 md:w-1/2" wire:key="{{ $criteria->id }}">
                <label class="label-text" for="number-input-label-{{ $criteria->id }}">Quantity:</label>

                <input wire:model="form.scores.{{ $criteria->id }}"
                    class="@error('form.scores.' . $criteria->id) is-invalid @enderror input"
                    id="number-input-label-{{ $criteria->id }}" type="number"
                    value="{{ old('form.scores.' . $criteria->id, $form->scores[$criteria->id] ?? 0) }}" min="0"
                    max="10" data-input-number-input />
